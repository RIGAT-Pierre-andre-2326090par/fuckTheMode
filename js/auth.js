...
function logoutUser() { localStorage.removeItem('currentUser'); window.location.href = "connexion.html"; }

function addOrderToHistory(totalAmount, itemsCount) {
    let currentUser = JSON.parse(localStorage.getItem('currentUser'));
    if (!currentUser) return;
    let newOrder = { id: '#' + Math.floor(Math.random()*9000+1000), date: new Date().toLocaleDateString(), total: totalAmount, count: itemsCount, status: "EN COURS" };
...
function saveProfilePicture(base64Image) {
    ...
    let users = JSON.parse(localStorage.getItem('usersDB'));
    let userIndex = users.findIndex(u => u.email === currentUser.email);
    if (userIndex !== -1) { users[userIndex].profilePic = base64Image; localStorage.setItem('usersDB', JSON.stringify(users)); }
    location.reload();
}
...
function changePassword(oldPass, newPass) {
    ...
    let users = JSON.parse(localStorage.getItem('usersDB'));
    let userIndex = users.findIndex(u => u.email === currentUser.email);
    if (userIndex !== -1) { users[userIndex].password = newPass; localStorage.setItem('usersDB', JSON.stringify(users)); }
